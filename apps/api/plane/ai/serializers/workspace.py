# InfraWatch — AI module workspace serializers
...
class WorkspaceAgentSerializer(serializers.ModelSerializer):
    ...
    skills = serializers.PrimaryKeyRelatedField(
        many=True,
        queryset=WorkspaceSkill.objects.all(),
        required=False,
    )
    tools = serializers.PrimaryKeyRelatedField(
        many=True,
        queryset=WorkspaceTool.objects.all(),
        required=False,
    )
    mcp_connections = serializers.PrimaryKeyRelatedField(
        many=True,
        queryset=WorkspaceMCPConnection.objects.all(),
        required=False,
    )
    ...
    class Meta:
        ...
        read_only_fields = ["source", "global_agent", "litellm_virtual_key"]
...
class WorkspaceSkillSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        read_only_fields = ["source", "global_skill"]
...
class WorkspaceToolSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        read_only_fields = ["source", "global_tool"]
...
class WorkspaceMCPConnectionSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        read_only_fields = ["source", "global_mcp"]
